...
if [ -f ".envrc" ]; then
  ...
  source .envrc
  ENVRC_LOADED=true
# Priority 2: Project .cursor/.envrc
elif [ -f ".cursor/.envrc" ]; then
  log "INFO" "Loading .envrc from .cursor/"
  source .cursor/.envrc
  ENVRC_LOADED=true
...
# Save raw JSON to temp file for Cursor to parse
echo "$body" > "${LOG_DIR}/${TICKET_KEY}.json"
log "INFO" "Raw JSON saved to: ${LOG_DIR}/${TICKET_KEY}.json"
...
echo "   📋 Log file:  $LOG_FILE"
echo ""
echo "🤖 Cursor AI can now parse these files for detailed requirements"
