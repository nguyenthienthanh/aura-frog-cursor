...
# Version: 1.0.0 (Adapted for Cursor IDE)
...
# Detect CURSOR_DIR - prioritize project's .cursor directory
if [ -d ".cursor" ]; then
  CURSOR_DIR=".cursor"
elif [ -d "$(pwd)/.cursor" ]; then
  CURSOR_DIR="$(pwd)/.cursor"
else
  # Fallback to plugin's directory if no project .cursor exists
  CURSOR_DIR="${PLUGIN_DIR}"
fi

# Set up paths
LOGS_DIR="${CURSOR_DIR}/logs"
WORKFLOWS_DIR="${LOGS_DIR}/workflows"
ACTIVE_WORKFLOW_FILE="${CURSOR_DIR}/active-workflow.txt"
...
archive_workflow() {
    local workflow_id="$1"
    local archive_dir="${CURSOR_DIR}/archive"
...
LOCATION:
    Workflows: .cursor/logs/workflows/
    Contexts:  .cursor/logs/contexts/
    Active:    .cursor/active-workflow.txt
EOF
}
