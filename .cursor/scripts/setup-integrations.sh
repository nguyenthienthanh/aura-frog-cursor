...
# Paths
CURSOR_DIR=".cursor"
ENVRC_FILE="$CURSOR_DIR/.envrc"
ENVRC_TEMPLATE="$CURSOR_DIR/.envrc.template"
PROJECT_ENVRC=".envrc"
...
# Phase 4: Generate .cursor/.envrc
echo -e "${BLUE}Phase 4: Generating configuration file...${NC}"
echo ""

# Create .envrc file
cat > "$ENVRC_FILE" << EOF
# .cursor/.envrc - CCPM Integration Configuration
...
# Phase 5: Setup project .envrc to load .cursor/.envrc
echo -e "${BLUE}Phase 5: Setting up project .envrc...${NC}"
echo ""

CCPM_LOADER="
# CCPM Integration (auto-generated)
if [ -f .cursor/.envrc ]; then
  source_env .cursor/.envrc
fi"

if [ -f "$PROJECT_ENVRC" ]; then
  if ! grep -q "source_env .cursor/.envrc" "$PROJECT_ENVRC"; then
...
if command -v direnv &> /dev/null; then
  ...
  echo "To load manually, run: source .cursor/.envrc"
fi
...
if [ -f ".cursor/scripts/test-integrations.sh" ]; then
  bash .cursor/scripts/test-integrations.sh
else
