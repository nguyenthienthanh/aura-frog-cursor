...
CONFLUENCE_URL=$(grep -A 5 "confluence:" .cursor/ccpm-config.yaml | grep "url:" | awk '{print $2}' | tr -d '"')
SPACE_KEY=$(grep -A 5 "confluence:" .cursor/ccpm-config.yaml | grep "space_key:" | awk '{print $2}' | tr -d '"')
