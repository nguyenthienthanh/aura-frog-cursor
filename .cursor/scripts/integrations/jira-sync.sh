...
if [ -z "$JIRA_EMAIL" ] || [ -z "$JIRA_API_TOKEN" ]; then
    ...
    echo "Set JIRA_EMAIL and JIRA_API_TOKEN in .cursor/.env"
    exit 1
fi

# Get JIRA config from ccpm-config.yaml
JIRA_URL=$(grep -A 5 "jira:" .cursor/ccpm-config.yaml | grep "url:" | awk '{print $2}' | tr -d '"')
PROJECT_KEY=$(grep -A 5 "jira:" .cursor/ccpm-config.yaml | grep "project_key:" | awk '{print $2}' | tr -d '"')
